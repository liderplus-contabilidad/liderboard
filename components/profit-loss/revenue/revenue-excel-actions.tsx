 ...
 *
 * **Here nothing is uploaded**: the revenue comes from PyG, so no `upload` is passed at all and no
 * upload button is drawn. It was rendered disabled with the reason in a pill first, and that is
 * precisely what the house rule forbids — a control that means nothing for the open data does not
 * render; the missing step belongs to another module, and the empty state is where it is named. The
 * one Excel that DOES come back in —the capture's own, «Datos registrados»— has its `ExcelActions`
 * inside the drawer that owns those figures, not in this bar.
 *
