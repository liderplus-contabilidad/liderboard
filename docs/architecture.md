# Arquitectura de liderboard — el porqué

Este archivo es el RELATO: por qué cada pieza es como es, qué se probó antes y por qué se descartó, y
las mediciones que sostienen una decisión (las separaciones ΔE de cada paleta, los anchos del
comprobante, el cuadre del asiento contra el libro del contador). Salió de `CLAUDE.md`, que lleva el
MAPA y los INVARIANTES y se carga en cada sesión: esto no se carga, se consulta.

No es una spec — las specs viven en `openspec/`. Es la memoria de las decisiones ya tomadas, así que
lo que se escribe aquí es el motivo, no la instrucción; cuando una decisión cambie, este archivo se
corrige en la misma tanda que el código, o deja de valer para nada.

Buena parte de este porqué vive TAMBIÉN —y a veces solo— en el docstring del archivo que lo aplica,
que es donde vincula: `lib/charts/palette.ts` para los tonos, `lib/payroll/journal.ts` para el
asiento, `lib/profit-loss/upload/` para cada sistema contable. Si los dos discrepan, manda el código.
...
dibuje igual que uno cargado en cero. El color de una barra de PAGADOR es su PUESTO y no su
identidad, la cuarta vez que el color deja de seguir a la entidad: diez entidades no caben en las
ocho ranuras de la paleta, y cada barra ya lleva su rótulo y su cifra. Ninguna vista recorre líneas
sueltas: `derive.ts` agrega antes por servicio, por pagador y por mes.
...
**El color de una barra de PAGADOR es su PUESTO, y arranca por los seis cálidos de la composición.**
Las diez iban de un solo azul —el color no distinguía nada que la fila no dijera— y después del set
DECORATIVO, y las dos veces se lee igual de mal: diez barras apagadas son una mancha justo donde hay
que recorrer un ranking. Toman un tono saturado cada una, como la tarjeta de arriba. Lo que NO pueden
tomar es el set de IDENTIDAD, y el motivo está en esta misma pantalla: ahí el color es la identidad
del SERVICIO —`movingServices` es la única lista de la que sale, así que «HONORARIOS» es el mismo azul
en la composición y en la pila de la evolución, donde una leyenda lo nombra—, y un primer pagador de
ese azul diría que ese pagador y ese servicio van juntos. Así que el ranking toma el otro set saturado
que la app ya midió: `CHART_SLICE_SEQUENCE`, los seis cálidos de la composición de PyG —ninguno es una
ranura de identidad— y detrás los decorativos, que es justo la forma de esta tarjeta: la cabeza del
ranking, que es lo que se lee, sale viva, y la cola paga el set decorativo con el mismo relieve que la
dona del anexo —cada barra lleva su rótulo y su cifra, y la gemela en tabla las lista todas—. El punto
de la tabla lo llevan solo las filas DIBUJADAS, porque uno en una fila que no está en el gráfico
promete una barra que el lector no va a encontrar.
